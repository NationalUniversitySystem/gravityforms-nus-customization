<?php
/**
 * Our custom Start Year select Field
 */
...
/**
 * Creates custom Start Year select field
 */
class Start_Year_Field extends GF_Field_Select {
	/**
	 * The field type
	 *
	 * @var string $type
	 */
	public $type = 'start_year';
	...
	public static function set_default_values() {
		...
		case "start_year" :
			...
			field.choices = new Array(
				new Choice("<?php echo esc_html( date( 'Y' ) ); ?>"),
				new Choice("<?php echo esc_html( date( 'Y', strtotime( '+1 year' ) ) ); ?>"),
				new Choice("<?php echo esc_html( date( 'Y', strtotime( '+2 year' ) ) ); ?>")
			)
			break;
		<?php
	}
	...
	public function custom_html( $field_content, $field, $value, $random = 0, $form_id ) {
		...
		if ( is_admin() || 'start_year' !== $field->type ) {
	...
	public static function custom_field_container( $field_container, $field, $form, $css_class, $style, $field_content ) {
		...
		$custom_classes .= 'form__group--year';
