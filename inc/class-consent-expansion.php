...
class Consent_Expansion {
	...
	public function __construct() {
		add_action( 'gform_field_appearance_settings', [ $this, 'consent_text_behavior_setting' ], 10, 2 );
	 ...
	 * @return void
	 */
	public function consent_text_behavior_setting( $position, $form_id ) {
		if ( 100 === $position ) {
			$behavior_options = [
	...
	public function modify_field_content( $field_content, $field, $value, $random = 0, $form_id ) {
		if ( is_admin() || 'consent' !== $field->type || empty( $field->consentTextBehavior ) ) { // phpcs:ignore WordPress.NamingConventions.ValidVariableName
